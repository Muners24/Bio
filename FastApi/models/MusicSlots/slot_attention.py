...
class SlotAttentionAE(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        # Obtener los optimizadores manualmente
        optimizers = self.optimizers()
        scheduler = self.lr_schedulers()

        # Asumiendo que solo tienes un optimizador (como en tu configure_optimizers)
        # Si tuvieras más, los tomarías como opt1, opt2, ...
        opt = optimizers if isinstance(optimizers, torch.optim.Optimizer) else optimizers[0]
